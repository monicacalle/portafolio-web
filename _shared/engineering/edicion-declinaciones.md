...
| 25 | Chapter I's intro video | §25 wants "a large video media block" as the first content beat after chapter I's intro, clickable, with a centred Play affordance. There is one film of her work and it is the Ceguera Digital campaign, which belongs to chapter III and is built there with §26's modal, §40's dark surround and §45's second beat. Chapter I is about five drawings nobody commissioned; there is no film of them, and showing the campaign film here as well would be the same repetition §69 is declined for. §25's mechanics are built where the film is: the media area is the click target, the Play affordance is centred and restrained, and the reveal is §28's. |
...
| 39 | Copy and media coexisting WITH the cinematic scene | §39 asks that "product copy and media should coexist with the cinematic scene", and the dark beat is a flat `--edicion-negro` rectangle: the editorial body under it is opaque cream and the persistent canvas is `position: fixed` behind both, so no part of chapter III's scene — which is built — is visible behind the film. Making the beat translucent would show the cream body, not the canvas; showing the canvas would mean punching a hole through an opaque body that four other chapters depend on being opaque, for one block. What §39's other three clauses ask for is built: the portion stays dark rather than moving to white, the feature enters with generous top padding, and the media is a nearly full-width presentation. The coexistence is the clause that is not, and it is not built rather than approximated. |
...
| 1 | Step 2 of §1.1's grammar, at the CHAPTER boundaries | Step 2 — "text begins to leave" — is built where the grammar's other six steps are: in the hero, where the standfirst and the index fade out over `--m` 0.45→0.72 as the frame travels, before the rail arrives. The hero's own label follows over `--p` 0.88→1, the anachronism over `--f` 0.72→1, the geometry over `--p` 0.62→0.90. This row used to claim the opposite — "NOTHING on this page animates on the way out", and that an exit "would be the only outbound animation in the build" — which the §17 block contradicts in as many words four rules away. What is genuinely not built is step 2 at the five CHAPTER boundaries: the chapter title enters (step 6) and does not leave. §78 is the reason, and it is a real one: it calls the light editorial blocks "a breathing mechanism" and warns against scroll-scrubbing them, so `Revelar` unobserves after revealing and the editorial world has no outbound animation at all. Six more scrubbed values, one per chapter title, for a moment the reader has already scrolled past. Steps 3, 4 and 5 carry what §1.1 is protecting: §77's three scrubbed cuts are what dissolve one chapter into the next. |
